...
def collect_docs_sources(repo_dir):
    ...
    release_revs = [rev for rev in remote_revs if rev.startswith(RELEASE_BRANCH_PREFIXES)]
    print(f"Release branches found: {release_revs}")
    for rev in release_revs:
        run_git("restore", "--source", rev, "--", DOCS_MD_SOURCE_PATHSPEC)
        docs_md_dir = DOCS_MD_DIR_TEMPLATE.format(release=rev[len(REMOTE) + 1:])
...
def build_docs_html(repo_dir):
    ...
    for release_dir in release_dirs:
        ...
        if not md_files:
            print(f"[WARNING] No validated markdown files in {release_dir}")
            continue
